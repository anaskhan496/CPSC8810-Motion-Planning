...
def compute_path(grid,start,goal,cost,heuristic):
    ...
    while len(open_set)!=0:
        current_node,current_node_add=open_set.pop()                            # This will pop the node and its address with least path cost
        if current_node==goal:                                                  # Check if the current node has reached the goal
            path[current_node[0]][current_node[1]]='*'                          # Make an asterisk mark on the goal
            ...
            while current_node!=init:                                           # This while loop will be used to back track the path traversed
                ...
                or_diff = parent_node[2] - current_node[2]                      # Calculate orientation difference
                ...
                if or_diff== -3:                                                # This special condition was given because the loop while backtracking turns from south to east which is right direction but it does not give the action corresponding to right
                    ...
                    or_diff = 1
                action_value = action.index(or_diff)                            # Will given the index of the action list
                act = action_name[action_value]                                 # Will give the corresponding action name to the above index
