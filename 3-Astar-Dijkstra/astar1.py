    ...
    while len(open_set)!=0:
        ...
        if current_node==goal:
            ...
            """while current_node!=start:
                if parent[current_node[2]][current_node[0]][current_node[1]]==' ':
                    continue
                parent_node = parent[current_node[2]][current_node[0]][current_node[1]]
                print(parent_node)
                or_diff = parent_node[current_node[2]][current_node[0]][current_node[1]][0] - current_node[2]
                act = action_name[or_diff]
                path[parent_node[0]][parent_node[1]] = act
                current_node = list(map(int, parent_node))"""
        closed_set.add(current_node)
    ...
    # Generate children
        ...
        for index,actions in enumerate(action): # Adjacent squares
            ...
            if t==-1:
                t=3
            # Get node position
            child_node = ( current_node[0]+forward[t][0], current_node[1]+forward[t][1],t)
            #print(node_position)
            ...
            if child_node[0] <= (len(grid)-1) and child_node[0] >= 0 and child_node[1] <= (len(grid[len(grid)-1])-1) and child_node[1] >= 0:
                ...
                if (grid[child_node[0]][child_node[1]]) == 0:
                    parent[t][child_node[0]][child_node[1]] = (current_node[2],current_node[0],current_node[1])
