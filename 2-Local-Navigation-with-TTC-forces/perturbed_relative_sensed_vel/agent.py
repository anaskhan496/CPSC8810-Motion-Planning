...
class Agent(object):
    ...
    def computeForces(self, neighbors=[]):
        ...
        samp_vels = np.random.uniform(low = 0, high = (nu)**2+0.001, size = N_samples)     #creating ramdomised velocities for sampled sensed velocity
        theta = np.random.uniform(low = 0, high = 2*(np.pi)+0.001, size = N_samples)       #creating randomised theta values for sampled sensed velocities
        samp_vels_x = np.sqrt(samp_vels)*np.cos(theta)                                     #x-coordinate for sampled sensed velocity
        samp_vels_y = np.sqrt(samp_vels)*np.sin(theta)                                     #y-coordinate for sampled sensed velocity
        candidate_vel[:,0] = samp_vels_x
        candidate_vel[:,1] = samp_vels_y
        eta = random.choice(candidate_vel)
        tau_not = 3
        k = 1
        m = 2
        ttc_min = float('inf')
        ...
        for i in range(len(neighbors)):
            if self.id != neighbors[i].id and not neighbors[i].atGoal:          #ignoring the same agent as its own neighbor
                ...
                if sense_dist < self.dhor:
                    ...
                    v = v + eta                                                 #perturbing the sensed relative velocity
                    a = v.dot(v) - eps**2
