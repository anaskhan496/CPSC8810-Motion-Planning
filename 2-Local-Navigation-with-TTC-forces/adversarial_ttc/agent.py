        ...
        for i in range(len(neighbors)):
            if self.id != neighbors[i].id and not neighbors[i].atGoal:          #ignoring the same agent as its own neighbor
                ...
                if sense_dist < self.dhor:
                    ...
                    v = v - eps*(rel_pos/(np.sqrt(rel_pos.dot(rel_pos))))      #adversarial uncertainty velocities
                    a = v.dot(v) - eps**2
                    ...
                    if ttc!=float('inf'):
                        if ttc<ttc_min:
                            ttc_min = ttc
                            n = (rel_pos + v*ttc_min)/np.sqrt((rel_pos + v*ttc_min).dot(rel_pos + v*ttc_min))
                            Fa = ((max((self.timehor-ttc_min),0))/(ttc_min))*n              #TTC based avoidance forces
                            Force = Force + Fa
                            if not self.atGoal:
                                self.F = Force
